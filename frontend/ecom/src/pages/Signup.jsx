...
import { Grid, Typography, FormControl, RadioGroup, FormControlLabel, Radio, Box, CircularProgress } from '@mui/material';
    ...
    const signup = async (data) => {
        try {
            ...
            if (response.type === "auth/signup/rejected") {
                toast.error(response.payload || "Signup failed. Please try again.");
                return;
            }
            if (!response.payload || !response.payload.user) {
                throw new Error('Failed to create account');
            }
            const { isVerified, role: userRole, token } = response.payload.user;
        ...
        <div className="w-full flex flex-col justify-center items-center mt-28">
            {isOtpTabVisible ? (
                ...
                <Box className="bg-white rounded-lg p-8 shadow-xl w-full max-w-md flex flex-col">
                    ...
                    <form onSubmit={handleSubmit(signup)} className="space-y-6" disabled={isLoading}>
